#!/usr/bin/env python3
"""
Run expanded within-category transfer evaluations with Gemini judge.

Tests ~22 new within-category pairs that haven't been tested before.
...
SKILL_PREFIX = "expand-within-gemini"
DEFAULT_ATTEMPTS = 3
...
# 22 new within-category pairs (all different from the 14 qwen-judged ones)
PAIRS = [
    # da-1 (oncology) - completely new domain
    ("da-1-3", "da-1-4"),
    # da-4 (oncology) - new pairs within da-4
    ("da-4-1", "da-4-6"),
    ("da-4-6", "da-4-7"),
    # da-8 (metabolic) - new pairs within da-8
    ("da-8-2", "da-8-1"),
    ("da-8-2", "da-8-3"),
    # da-13 (metabolic) - new pairs within da-13
    ("da-13-1", "da-13-3"),
    ("da-13-3", "da-13-5"),
    ("da-13-3", "da-13-6"),
    ("da-13-5", "da-13-1"),
    # da-14 (immunology) - new pairs within da-14
    ("da-14-1", "da-14-3"),
    ("da-14-3", "da-14-1"),
    ("da-14-3", "da-14-8"),
    # da-15 (neurology) - new pairs within da-15
    ("da-15-1", "da-15-2"),
    ("da-15-2", "da-15-7"),
    ("da-15-2", "da-15-8"),
    # da-17 (immunology) - new pairs within da-17
    ("da-17-1", "da-17-3"),
    # da-18 (oncology) - new pairs within da-18
    ("da-18-1", "da-18-5"),
    ("da-18-1", "da-18-7"),
    # da-19 (oncology) - new pairs within da-19
    ("da-19-1", "da-19-3"),
    ("da-19-1", "da-19-4"),
    ("da-19-1", "da-19-6"),
    # da-20 (general-biology) - new pair within da-20
    ("da-20-1", "da-20-3"),
]
...
def deploy_skill(source: str, target: str) -> tuple[bool, str]:
    ...
    metadata = {
        ...
        "experiment": "expand_within_category",
    }
...
def make_env() -> dict[str, str]:
    env = os.environ.copy()
    key = env.get("ANTHROPIC_API_KEY", "00gcclg9l39y9p01000dhjzolag1q2hk00901kh1")
    env["ANTHROPIC_API_KEY"] = key
    env["ANTHROPIC_BASE_URL"] = env.get("ANTHROPIC_BASE_URL", "https://api.gpugeek.com")
    env["ANTHROPIC_MODEL"] = SOLVER_MODEL
    env["QWEN_API_KEY"] = env.get("QWEN_API_KEY", key)
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            ...
            if attempt < attempts and is_retryable_failure_text(failure_text):
                print(f"[RETRY] {label} retryable failure, sleeping {RETRY_BACKOFF_SECONDS}s", flush=True)
                time.sleep(RETRY_BACKOFF_SECONDS)
            elif attempt < attempts:
                print(f"[RETRY] {label} non-retryable failure, will retry anyway", flush=True)
                time.sleep(30)
            ...
            if attempt < attempts:
                print(f"[RETRY] {label} timeout, retrying after {RETRY_BACKOFF_SECONDS}s", flush=True)
                time.sleep(RETRY_BACKOFF_SECONDS)
    return last_result


def main() -> int:
    parser = argparse.ArgumentParser(description="Expanded within-category transfer evaluation (Gemini judge)")
    parser.add_argument("--dry-run", action="store_true", help="Dry run, only deploy and print config")
    parser.add_argument("--max-concurrent", type=int, default=2, help="Max concurrent pairs")
    parser.add_argument("--attempts", type=int, default=DEFAULT_ATTEMPTS, help="Max retry attempts per pair")
    ...
    print("=" * 60)
    print("Expanded Within-Category Transfer (Gemini judge)")
    ...
    summary_path = LOGS_DIR / f"expand_within_summary_{timestamp}.json"
