...
class V10Config:
    # ── Server Paths ──────────────────────────────────────────────────────
    remote_base: str = field(default_factory=lambda: os.environ.get("SKILL_TRANSFER_ROOT", "/data/yjh/skill-transfer-eval"))
    ...
    bio_dir: str = field(default_factory=lambda: os.environ.get("BIOMNIBENCH_TASKS_DIR", "/data/yjh/biomnibench-organized"))
    runs_dir: str = field(init=False)
    transfer_dir: str = field(init=False)

    # ── Harness / CLI ────────────────────────────────────────────────────
    harness_dir: str = field(default_factory=lambda: os.environ.get("BIOMNIBENCH_HARNESS_DIR", "/tmp/my_claude_biomnibench_fixed"))
    bun_bin: str = field(default_factory=lambda: os.environ.get("BUN_BIN", "bun"))
