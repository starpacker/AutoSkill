...
# Use Python to parse ps output instead of awk to avoid escaping issues
kill_out = run('ps aux | grep "bun.*harness.*skillopt" | grep -v "grep"', 30)
print(f"bun processes found: {kill_out}")
...
print(run(f'echo {encoded} | base64 -d > /data/yjh/skill-opt/repo/run_skillopt.sh && chmod +x /data/yjh/skill-opt/repo/run_skillopt.sh'))

# Step 5: Verify script
print('=== Step 5: Verify script ===', flush=True)
print(run('cat /data/yjh/skill-opt/repo/run_skillopt.sh'))

# Step 6: Clean old output dir
print('=== Step 6: Clean old output dir ===', flush=True)
...
print('=== Step 7: Restart in tmux ===', flush=True)
